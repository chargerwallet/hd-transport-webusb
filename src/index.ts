...
import transport, { AcquireInput, LogBlockCommand } from '@chargerwallet/hd-transport';
...
export default class WebUsbTransport {
  ...
  async _getDeviceList() {
    ...
    const chargerwalletDevices = devices.filter(dev => {
      const isChargerWallet = CHARGERWALLET_FILTER.some(
  ...
  async requestDevice() {
    if (!this.usb) return;
    try {
      const device = await this.usb.requestDevice({ filters: CHARGERWALLET_FILTER });
      return device;
    } catch (e) {
